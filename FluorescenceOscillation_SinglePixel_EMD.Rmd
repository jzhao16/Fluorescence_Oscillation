...
names(df_pixel_dt) <- "Intensity"
tt <- seq(1,dim(df_pixel_dt)[1],1)
# df_pixel_dt[["Frame"]] <- tt

# Conduct EMD spectrum decomposition
emd_s <- emd(df_pixel_dt$Intensity, tt, boundary="wave", max.imf = 6)

# Conduct Hilbert-Huang Transformation
emd_f <- hilbertspec(emd_s$imf)

### Ploting the IMF's

for(i in 1:emd_s$nimf) {
p1 <- ts_ggplot(emd_s$imf[,i], paste(i, "-th IMF", sep=""))
p2 <- ts_histo_freq_ggplot(emd_f$instantfreq[,i], paste(i, "-th Instant Freqeuncy", sep=""))
grid.arrange(p1,p2, ncol=1)
}

p1 <- ts_ggplot(emd_s$residue, paste("residue", sep=""))
...
ts_histo_freq_ggplot <- function(ts, title) {
  nbins = 125
  maxFreq = 500
  df <- data.frame(ts) * 1000000/frame_rate
  names(df) <- "InstantFrequency"
  title <- paste(title, round(mean(df$InstantFrequency),digits=2), "±", round(sd(df$InstantFrequency),digits=2), "mHz")
  p1 <- ggplot(df) + geom_histogram(aes(x = InstantFrequency, y = ..density..), color="gray", fill="steelblue3", bins = nbins) +  
    stat_function(
		fun = dnorm, 
		args = with(df, c(mean = mean(InstantFrequency), sd = sd(InstantFrequency)))) + xlab("Instant Frequency (mHz)") + ggtitle(title) + scale_x_continuous(limits = c(0, maxFreq), sec.axis = sec_axis(~ .*frame_rate/1000000, name="Instant Frequency (per frame)"))
  return (p1)
}

ts_histo_ggplot(emd_f$instantfreq[,5], paste(5, "-th Instant Freqeuncy,", sep=""))


```
```{r}

ts_histo_amp_ggplot <- function(ts, title) {
  nbins = 50
  maxAmp = 250
  df <- data.frame(ts)
  names(df) <- "InstantAmplitude"
  title <- paste(title, round(mean(df$InstantAmplitude),digits=2), "±", round(sd(df$InstantAmplitude),digits=2))
  p1 <- ggplot(df) + geom_histogram(aes(x = InstantAmplitude, y = ..density..), color="gray", fill="steelblue3", bins = nbins) + xlab("Instant Ampitude") + ggtitle(title) + scale_x_continuous(limits = c(0, maxAmp))
  return (p1)
}

ts_histo_amp_ggplot(emd_f$amplitude[,5], paste(5, "-th Instant Amplitude,", sep=""))

```
```{r}
for(i in 1:emd_s$nimf) {
p1 <- ts_ggplot(emd_s$imf[,i], paste(i, "-th IMF", sep=""))
p2 <- ts_histo_amp_ggplot(emd_f$amplitude[,i], paste(i, "-th Instant Amplitude", sep=""))
grid.arrange(p1,p2, ncol=1)
}
```
...
screen_criteria <- function(per, r.square){
  if ((per < period_lowerboundary) | (per > period_upperboundary)){
    ...
    if (r.square < r.square_threshold){
      ...
      return(c(round(per, digit = 2), "Detected signal pass the screening criteria, period is recorded"))
    }
  }
}

acf_sine_plot <-function (ts, header) {
  ...
  title <- paste(
    ...
    "Period = ", round(per, digits = 2), " frames, ", 
    "Period = ", round(per * frame_rate/1000, digits = 2), " s, ", 
...
for(i in 1:try$nimf) {
print(acf_sine_plot(try$imf[,i], paste(i, "-th IMF", sep="")))
}
