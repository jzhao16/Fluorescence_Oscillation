...
df_stats_BG <- read.csv("BG-2_stats.csv", row.names = 1)
...
#img_ROI <- rgbImage( blue=matrix_ROI, green=matrix_Peripheral )
...
matrix_amp <- as.matrix.xtabs(xtabs( Amplitude ~ X+Y, data=df_stats_heatmap))/60
matrix_amp <- normalize(matrix_amp)

# Cut-off background pixel's period value (with Cell outline mask)
df_stats_heatmap[df_stats_heatmap$Indicator == 1,]$Amplitude <- 0

matrix_amp_mask <- as.matrix.xtabs(xtabs( Amplitude ~ X+Y, data=df_stats_heatmap))/60
...
## Plotting Statistics
**Histogram**
```{r, warning=FALSE, fig.height = 4, fig.width = 8,fig.align="center",timeit = TRUE, eval=FALSE}
...
**Scatter Plot**
```{r,warning=FALSE, fig.height = 7, fig.width = 7, fig.align="center",timeit = TRUE, eval=FALSE}
...
**Barplot**
```{r, warning=FALSE, fig.height = 4, fig.width = 6, fig.align="center",timeit = TRUE, eval=FALSE}
...
ggplot(df_cmd[df_cmd$Amplitude!=0,], aes(x=Amplitude))  + scale_x_continuous(limits = c(0, 90)) + 
