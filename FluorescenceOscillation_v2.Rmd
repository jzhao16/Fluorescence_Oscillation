...
for (i in 1:length(df2)) {
  ...
  df_stats[i,'A1'] <- round(mean(abs(F1)),digits=2)
  df_stats[i,'A2'] <- round(mean(abs(F2)),digits=2)
  df_stats[i,'A3'] <- round(mean(abs(F3)),digits=2)
  df_stats[i,'A4'] <- round(mean(abs(F4)),digits=2)
...
amplitude_sel <- function(df_stats){
  if (df_stats[18] != 0){
      ...
      df_stats[19] <- max(df_stats[5],df_stats[8],df_stats[11],df_stats[14])
  }
}
...
```{r,warning=FALSE, fig.height = 4, fig.width = 7, fig.ncol=2, fig.align="center",eval = FALSE}
# Make a df_stats copy for factors plotting
df_stats_factor <- df_stats[df_stats$Period!=0,]

df_stats_factor$Period <- cut(df_stats_factor$Period, c(7, 20, 40, 60, 80, 100, 120))
...
p11 <- ggplot(df_stats_factor_Cell, aes(y = Amplitude, x = Period)) + geom_boxplot(fill='blue',alpha=0.6) + scale_y_continuous(limits = c(5, 62)) + 
    stat_summary(fun.y=mean, geom="point", shape=5, size=2) + ggtitle("Cell ROI") + ylab("Average of Fluorescence Intensity") + xlab("Period (Frames)")
 
p12 <- ggplot(df_stats_factor_BG, aes(y = Amplitude, x = Period)) + geom_boxplot(fill='red',alpha=0.6) + scale_y_continuous(limits = c(5, 62)) +
    stat_summary(fun.y=mean, geom="point", shape=5, size=2) + ggtitle("Background") + ylab("Average of Fluorescence Intensity") + xlab("Period (Frames)")

p13 <- ggplot(df_stats_factor_RI, aes(y = Amplitude, x = Period)) + geom_boxplot(fill='green',alpha=0.6) + scale_y_continuous(limits = c(5, 62)) +
    stat_summary(fun.y=mean, geom="point", shape=5, size=2) + ggtitle("Peripheral") + ylab("Average of Fluorescence Intensity") + xlab("Period (Frames)")
