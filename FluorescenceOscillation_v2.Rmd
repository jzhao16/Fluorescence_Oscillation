...
sine.screen.plot <- function(ts, per, threshold, header) {
  ...
  # The sinusoid screening criteria !!! 
  if (summary(reslm)$r.squared >= threshold & (per < 120) & (per > 8)) {
    ...
    p_spec <- ggplot(df_ssp, aes(x=Period, y=Spec)) + geom_bar(stat="identity", color="black") + xlab("Period (Frame)") + ylab("Spectral Density") + ggtitle(paste(header, "Spectral Periodgram,","Period =", signif(per,digits=4)))
...
#df_stats_heatmap[df_stats_heatmap$Period == 9.1,]$Period <- 1
#df_stats_heatmap[df_stats_heatmap$Period == 10.7,]$Period <- 2
#df_stats_heatmap[df_stats_heatmap$Period == 12.8,]$Period <- 3
#df_stats_heatmap[df_stats_heatmap$Period == 16.0,]$Period <- 4
#df_stats_heatmap[df_stats_heatmap$Period == 21.3,]$Period <- 5
#df_stats_heatmap[df_stats_heatmap$Period == 32,]$Period <- 6
...
df_period_cmd_new <- melt(df_period_cmd[2:5,], id = "Period")
